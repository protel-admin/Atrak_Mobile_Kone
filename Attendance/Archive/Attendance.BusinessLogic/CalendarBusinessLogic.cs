...
namespace Attendance.BusinessLogic
{
    public class CalendarBusinessLogic
    {
        public List<CalendarDays> GetCalendarDays(int month, int year,string staffid)
        {
            ...
            {
                calendarDays = calendarRepository.GetShiftCalendar(StartDate.ToString("MMMM").ToUpper(), StartDate.Year, staffid);
            }
            //bring the shift calendar into a proper calendar.
            var lstCD = DrawCalendar(calendarDays, StartDate);
            ...
            for ( d = 1 ; d <= 42 ; d += 1 ) {
                ...
                if ( d >= ( Convert.ToInt16 ( StartDate.DayOfWeek ) +1) ) {
                    if ( loadday == true ) {
                        ...
                        if ( c != null ) { //if shift found for the day then...
                            ...
                            Cd.ActualDate = textInfo.ToTitleCase(c.ActualDate).Replace(' ','-');
                            ...
                            if (c.ActualInTime != null)
                                Cd.ActualInTime = textInfo.ToTitleCase(c.ActualInTime).Replace(' ', '-');
                            else
                                Cd.ActualInTime = "--";
                            ...
                            if (c.ActualOutTime != null)
                                Cd.ActualOutTime = textInfo.ToTitleCase(c.ActualOutTime).Replace(' ', '-');
                            else
                                Cd.ActualOutTime = "--";
