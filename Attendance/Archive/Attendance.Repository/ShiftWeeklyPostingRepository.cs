...
{
    public class ShiftWeeklyPostingRepository:TrackChangeRepository
    {
        ...
        public List<ShiftPostingPatternList> GetShiftPostingPatternList(int PatternId)
        {
            ...
            try
            {
                var lst = context.Database.SqlQuery<ShiftPostingPatternList>(QryStr.ToString(),new SqlParameter("@PatternId",PatternId)).Select(d => new ShiftPostingPatternList()
                {
                    Id = d.Id,
