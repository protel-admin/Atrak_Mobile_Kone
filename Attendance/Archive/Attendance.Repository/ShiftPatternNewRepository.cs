...
namespace Attendance.Repository
{
    public class ShiftPatternNewRepository : IDisposable
    {
        ...
        public void SaveInformation(ShiftPatternNewList spnl)
        {
            using (DbContextTransaction Trans = context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    context.Database.ExecuteSqlCommand("delete from shiftpatterntxn where PatternId = " + sp.Id);
        ...
        public List<ShiftList> GetShiftList(int Id)
        {
            ...
            try
            {
                var lst = context.Database.SqlQuery<ShiftList>(qryStr.ToString(),new SqlParameter("@Id",Id)).Select(d => new ShiftList()
                {
                    ShiftId = d.ShiftId,
