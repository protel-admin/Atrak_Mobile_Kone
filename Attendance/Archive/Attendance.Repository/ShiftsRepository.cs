    ...
    public class ShiftsRepository : IDisposable
    {
        ...
        public List<ShiftView> GetAllShifts(string role, string Location)
        {
            ...
            qryStr.Append("select id , name , shortname , " +
                            ...
                            "isactive , CreatedOn , CreatedBy, LocationId ,SAPShiftName from shifts union  select  'LV0011'  as id , 'WEEKLY OFF' as name,'WO' as shortname,  '00:00' as starttime , '00:00' as endtime ,'' as gracelateby,'' as graceearlyby,'' as " +
                            "breakstarttime ,'' as breakendtime,'' as mindayhours,'' as minweekhours,'' as isactive,'' as CreatedOn,'' as CreatedBy, '' as LocationId ,'' as SAPShiftName");
            }
            else
            {
                qryStr.Append("select id , name , shortname , " +
                                           ...
                                           "convert ( varchar , mindayhours ) as  mindayhours, convert ( varchar , minweekhours ) as minweekhours , " +
                                           "case when isactive = 1 then 'Yes' else 'No' end as " +
                                           "isactive , CreatedOn , CreatedBy, LocationId, SAPShiftName from shifts where LocationId= @Location union  select  'LV0011'  as id , 'WEEKLY OFF' as name,'WO' as shortname,  '00:00' as starttime , '00:00' as endtime ,'' as gracelateby,'' as graceearlyby,'' as " +
                                           "breakstarttime ,'' as breakendtime,'' as mindayhours,'' as minweekhours,'' as isactive,'' as CreatedOn,'' as CreatedBy, '' as LocationId,'' as SAPShiftName ");
            }
            ...
            var lstSh = context.Database.SqlQuery<ShiftView>(qryStr.ToString(),new SqlParameter("@Location", Location)).Select(d => new ShiftView()
            {
                Id = d.Id,
                Name = d.Name,
                ShortName = d.ShortName,
                SAPShiftName = d.SAPShiftName,
        ...
        public string SaveShiftExtensionDetails(ShiftExtensionAndDoubleShift se)
        {
            ...
            using (var transaction = context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    AttendanceControlTable ACT = new AttendanceControlTable();
                    ACT.StaffId = se.StaffId;
                    ACT.FromDate = se.TxnDate;
                    ACT.ToDate = se.TxnDate;
                    ACT.IsProcessed = false;
                    ACT.CreatedOn = DateTime.Now;
                    ACT.CreatedBy = se.CreatedBy;
                    ACT.ApplicationType = "SE";
                    ACT.ApplicationId = MaxId;
                    context.AttendanceControlTable.Add(ACT);
                ...
                //////////    Reviwer = repo.GetStaffReviewer(se.StaffId);
                //////////    if (ReportingManager == null && ReportingManager == "")
                //////////    {
                //////////        throw new Exception("ReportingManager not configer for given empid: "+ se.StaffId +"");
                //////////    }
                //////////    if (Reviwer == null && Reviwer == "")
                //////////    {
                //////////        throw new Exception("Reviwer not configer for given empid: " + se.StaffId + "");
                //////////    }
                //////////    queryString1.Clear();
                ...
                //////////    AAobj.Comment = "-";
                //////////    AAobj.ApprovalStatusId = 2;
                //////////    AAobj.ApprovalOwner = ReportingManager;
                //////////    AAobj.ApprovedBy = ReportingManager;
                //////////    AAobj.ApprovedOn = DateTime.Now;
                //////////    AAobj.ReviewerstatusId = 2;
                //////////    AAobj.ReviewerOwner = Reviwer;
                //////////    AAobj.ReviewedBy = Reviwer;
                //////////    AAobj.ReviewedOn = DateTime.Now;
