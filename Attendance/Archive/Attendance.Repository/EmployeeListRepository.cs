    ...
    public class EmployeeListRepository : IDisposable
    {
        ...
        public List<EmployeeList> LoadListOfEmployees()
        {
            ...
            qryStr.Append("SELECT StaffId ,FirstName ,DeptName ,DesignationName "+
                ",GradeName ,StatusName ,convert ( varchar ,WorkingPattern) AS WorkingPattern FROM [StaffView] ORDER BY [FirstName]");
        ...
        public List<EmployeeList> LoadAlphabetWiseListOfEmployees(char alphabet)
        {
            ...
            sqlParameter[0] = new SqlParameter("@alphabet", alphabet);
            var qryStr = new StringBuilder();
            qryStr.Clear();
            qryStr.Append("SELECT StaffId ,FirstName ,DeptName ,DesignationName " +
                ",GradeName ,StatusName ,convert ( varchar ,WorkingPattern) AS WorkingPattern "+
            ...
            try
            {
                var lst = context.Database.SqlQuery<EmployeeList>(qryStr.ToString(),sqlParameter).Select(d => new EmployeeList()
                {
                    StaffId = d.StaffId,
                    FirstName = d.FirstName,
                    DeptName = d.DeptName,
