...
namespace Attendance.Repository
{
    public class StaffListRepository : IDisposable
    {
        ...
        public List<string> GetStaffsByReportingManager(string reportingManager)
        {
            ...
            query.Append("Select Distinct UPPER(StaffId) from [StaffOfficial] where  ReportingManager = @reportingManager" +
                " AND StaffId in (Select Distinct Id from [Staff] where StaffStatusId = 1)  ");
        ...
        public List<StaffList> GetStaffBasedonrep(string criteria, string ReportingManager)
        {
            ...
            if (staff.Contains(criteria) == true)
            {
                try
                {
                    ...
                    qryStr.Append("Select staffid , DBO.FNGETSTAFFNAME(STAFFID) as staffname , DBO.FNGETMASTERNAME(STAFFID , 'DP') as " +
                        "Department, DBO.FNGETMASTERNAME(STAFFID , 'L') as Location from StaffOfficial a inner join Staff b on " +
                        "a.staffid = b.id WHERE 1=1   AND b.StaffStatusId = 1 AND  staffid = @criteria");
        ...
        public List<StaffList> GetStaffBasedonrep(string ReportingManager)
        {
            ...
            foreach (var st in staff)
            {
                try
                {
                    ...
                    qryStr.Append("Select staffid , DBO.FNGETSTAFFNAME(STAFFID) as staffname , DBO.FNGETMASTERNAME(STAFFID , 'DP') as" +
                        " Department, DBO.FNGETMASTERNAME(STAFFID , 'L') as Location from StaffOfficial a inner join Staff b on " +
                        "a.staffid = b.id WHERE 1=1 AND b.StaffStatusId = 1 AND  staffid = @st");
